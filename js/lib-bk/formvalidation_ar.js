...
jQuery( "#email-form" ).validate({
    ...
    messages: {
        ...
        //},
        newemail: "نرجو كتابة البريد الإلكتروني الصحيح",
        confirmnewemail: "نرجو كتابة البريد الإلكتروني الصحيح",
    ...
    jQuery( "#confirmnewemail" ).blur(function() {
        if ((/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(jQuery("#confirmnewemail").val())))
        {
            if(jQuery("#confirmnewemail").val() != jQuery("#newemail").val())
            {
                ...
                jQuery("#confirmnewemail").after( '<label id="confirmnewemail-error" class="error" for="confirmnewemail">نرجو كتابة نفس البريد الالكتروني</label>' );
                return false;
            }
        }
    });
